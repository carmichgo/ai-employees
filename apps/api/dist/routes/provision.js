...
export async function provisionRoutes(fastify) {
    ...
    fastify.get("/internal/employees/:id/channels/whatsapp/qr", async (request, reply) => {
        ...
        try {
            ...
            const sessionCheck = execSync(`docker exec ${containerTarget} bash -c 'test -f /home/node/.openclaw/credentials/whatsapp/creds.json && echo "linked" || echo "unlinked"'`, { timeout: 5000 }).toString().trim();
            ...
            const output = execSync(`docker exec ${containerTarget} timeout 15 node -e "
        ...
        " 2>/dev/null`, { timeout: 20000 }).toString().trim();
