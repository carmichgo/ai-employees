...
import { createEmployeeSchema, updateEmployeeSchema, getJobTemplate, getModelForTier, } from "@ai-employees/shared";
import { getProvisionQueue } from "../queues.js";
export async function employeeRoutes(fastify) {
    // List all employees for company
    fastify.get("/api/employees", { onRequest: [fastify.authenticate] }, async (request) => {
    ...
    fastify.get("/api/employees/:id", { onRequest: [fastify.authenticate] }, async (request, reply) => {
        const { companyId } = request.user;
    ...
    // Hire a new employee
    fastify.post("/api/employees", { onRequest: [fastify.authenticate] }, async (request, reply) => {
        const { companyId } = request.user;
    ...
    // Update employee
    fastify.patch("/api/employees/:id", { onRequest: [fastify.authenticate] }, async (request, reply) => {
        const { companyId } = request.user;
        const { id } = request.params;
        ...
        if (input.persona !== undefined)
            updateData.persona = input.persona;
    ...
    // Pause employee
    fastify.post("/api/employees/:id/pause", { onRequest: [fastify.authenticate] }, async (request, reply) => {
        const { companyId } = request.user;
        const { id } = request.params;
        const employee = await db.query.employees.findFirst({
            where: and(eq(employees.id, id), eq(employees.companyId, companyId)),
    ...
    // Resume employee
    fastify.post("/api/employees/:id/resume", { onRequest: [fastify.authenticate] }, async (request, reply) => {
        const { companyId } = request.user;
        const { id } = request.params;
        const employee = await db.query.employees.findFirst({
            where: and(eq(employees.id, id), eq(employees.companyId, companyId)),
    ...
    // Terminate employee
    fastify.delete("/api/employees/:id", { onRequest: [fastify.authenticate] }, async (request, reply) => {
        const { companyId } = request.user;
        const { id } = request.params;
        const employee = await db.query.employees.findFirst({
            where: and(eq(employees.id, id), eq(employees.companyId, companyId)),
    ...
    fastify.get("/api/employees/templates", async () => {
        const { JOB_TEMPLATES, getJobTemplateCategories } = await import("@ai-employees/shared");
        return {
