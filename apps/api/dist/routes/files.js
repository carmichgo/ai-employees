...
import { mkdirSync, writeFileSync, readdirSync, readFileSync, unlinkSync, statSync, existsSync, } from "node:fs";
import { execSync } from "node:child_process";
import path from "node:path";
const CONFIG_BASE = "/opt/ai-employees/openclaw-configs";
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10 MB
function getUploadsDir(employeeId) {
...
export async function fileRoutes(fastify) {
    ...
    fastify.post("/internal/employees/:id/files", async (request, reply) => {
    ...
    fastify.get("/internal/employees/:id/files", async (request, reply) => {
        const { id } = request.params;
        const employee = await db.query.employees.findFirst({
            where: eq(employees.id, id),
        });
    ...
    fastify.get("/internal/employees/:id/files/:filename", async (request, reply) => {
    ...
    fastify.get("/internal/employees/:id/workspace/*", async (request, reply) => {
        const { id } = request.params;
    ...
    fastify.delete("/internal/employees/:id/files/:filename", async (request, reply) => {
        const { id, filename } = request.params;
        const uploadsDir = getUploadsDir(id);
        const filePath = path.join(uploadsDir, sanitizeFilename(filename));
        if (!existsSync(filePath)) {
