...
function emojiToSlackIcon(emoji) {
    if (!emoji)
        return undefined;
...
export class SlackProxy {
    ...
    async start() {
        ...
        try {
            ...
            const bolt = await import(/* webpackIgnore: true */ boltMod);
            BoltApp = bolt.App;
            LogLevel = bolt.LogLevel;
            const webApi = await import(/* webpackIgnore: true */ webApiMod);
        ...
        this.botUserId = slack.botUserId || null;
    ...
    async refreshMappings() {
        ...
        const emps = await db.query.employees.findMany({
            where: and(eq(employees.companyId, this.companyId), eq(employees.status, "active")),
        });
        this.channelToEmployee.clear();
        for (const emp of emps) {
            const accounts = emp.provisionedAccounts || {};
            const slackInfo = accounts.slack;
            const channelId = slackInfo?.channelId;
            const mapping = {
                id: emp.id,
                name: emp.name,
                emoji: emp.emoji,
     ...
     */
    async createEmployeeChannel(employeeId) {
        if (!this.webClient)
            return null;
    ...
    async findExistingChannel(channelName, employeeId, emp) {
        if (!this.webClient)
            return null;
        try {
    ...
    async archiveEmployeeChannel(employeeId) {
        if (!this.webClient)
            return;
        const emp = await db.query.employees.findFirst({
            where: eq(employees.id, employeeId),
     ...
     */
    async fetchChannelHistory(channelId, beforeTs) {
        if (!this.webClient)
            return [];
        try {
            ...
            for (const msg of slackMessages) {
                ...
                if (msg.subtype && msg.subtype !== "bot_message" && msg.subtype !== "file_share")
                    continue;
    ...
    async handleMessage(message, client) {
        ...
        if (!freshEmp || freshEmp.status !== "active")
            return;
        employee = {
            ...employee,
        ...
        try {
            ...
            try {
                ...
                if (containerName) {
                    try {
                        const newIp = execSync(`docker inspect --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' ${containerName}`, { timeout: 5000 }).toString().trim();
    ...
    async postAsEmployee(client, channel, employee, text) {
        try {
    ...
    async reconcileChannels() {
        if (!this.companyId || !this.webClient)
            return [];
        const emps = await db.query.employees.findMany({
            where: and(eq(employees.companyId, this.companyId), eq(employees.status, "active")),
        });
        ...
        for (const emp of emps) {
            ...
            if (slackInfo?.channelId) {
                results.push({ name: emp.name, channelId: slackInfo.channelId });
                continue;
            }
     ...
     */
    async announceNewEmployee(emp, channelId) {
        if (!this.webClient)
            return;
        try {
            // Find #general (or fall back to first public channel)
            const list = await this.webClient.conversations.list({
                types: "public_channel",
                limit: 200,
            });
            const general = list.channels?.find((c) => c.name === "general" && !c.is_archived);
            if (!general?.id) {
    ...
    async postMessageAsEmployee(employeeId, channelId, text) {
        if (!this.webClient)
            return false;
        const emp = await db.query.employees.findFirst({
            where: eq(employees.id, employeeId),
