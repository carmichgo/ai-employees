  ...
  fastify.get("/debug", async () => {
    ...
    try {
      checks.waFiles = execSync(
        "docker exec $(docker ps -q --latest) bash -c 'find /home/node/.openclaw -maxdepth 4 -name \"*.json\" 2>/dev/null | grep -i \"wa\\|whatsapp\\|cred\" | head -30; echo \"---\"; ls -la /home/node/.openclaw/credentials/ 2>/dev/null; echo \"---\"; cat /home/node/.openclaw/wa-qr-status.json 2>/dev/null; echo \"---\"; tail -30 /home/node/.openclaw/wa-qr.log 2>/dev/null'",
        { timeout: 10000 },
      ).toString().trim();
