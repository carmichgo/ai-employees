...
export async function buildServer(config: Env) {
  ...
  fastify.get("/debug", async () => {
    ...
    try {
      ...
      if (cid) {
        checks.waFiles = execSync(
          `docker exec ${cid} find /home/node/.openclaw -maxdepth 4 -type f -name '*.json' 2>/dev/null`,
          { timeout: 10000 },
        ).toString().trim();
        try {
          checks.waStatus = execSync(
            `docker exec ${cid} cat /home/node/.openclaw/wa-qr-status.json 2>/dev/null || echo none`,
            { timeout: 5000 },
          ).toString().trim();
        } catch { checks.waStatus = "none"; }
        try {
          checks.waLog = execSync(
            `docker exec ${cid} cat /home/node/.openclaw/wa-qr.log 2>/dev/null || echo none`,
            { timeout: 5000 },
          ).toString().trim();
        } catch { checks.waLog = "none"; }
        try {
          checks.openlawConfig = execSync(
            `docker exec ${cid} cat /home/node/.openclaw/openclaw.json 2>/dev/null | head -c 3000 || echo none`,
            { timeout: 5000 },
          ).toString().trim();
        } catch { checks.openlawConfig = "none"; }
      }
    } catch (e: any) { checks.waFiles = `error: ${e.message}`; }
    return checks;
  });
