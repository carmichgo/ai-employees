  ...
  const callSendAndRespond = useCallback(async (text: string) => {
    ...
    try {
      ...
      try {
        ...
        // speakAsync handles its own errors
      }
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {employee.status === "active" && (
            <button
              onClick={async () => {
                ...
                try {
                  const res = await api.restartEmployee(employeeId, true);
                  setMessages([{
                    id: "welcome",
                    role: "assistant",
          ...
          <button
            onClick={async () => {
              ...
              try {
                await api.clearChatHistory(employeeId);
                setMessages([{
                  id: "welcome",
                  role: "assistant",
                  content: `Hi! I'm ${employee.name}, your ${employee.jobTitle}. How can I help you today?`,
                  timestamp: new Date(),
                }]);
              } catch (err: any) {
