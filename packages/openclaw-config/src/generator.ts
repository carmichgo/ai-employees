...
export function generateSoulMd(employee: EmployeeInput): string {
  ...
  parts.push("```");
  parts.push("");
  parts.push("**Update/complete a task:**");
  parts.push("```bash");
  ...
  parts.push("```");
  parts.push("");
  parts.push("**Priority values:** `low`, `medium`, `high`, `urgent`");
  parts.push("**Status values:** `pending`, `in_progress`, `completed`, `blocked`");
  parts.push("**Category examples:** `research`, `marketing`, `engineering`, `content`, `admin`, `support`, `outreach`");
  parts.push("");
  parts.push("**Task lifecycle:** (1) Receive work → (2) Create task with `in_progress` → (3) Do the work → (4) Update to `completed` with a comment → (5) Report back. If blocked, update to `blocked` with a comment explaining what you need.");
  parts.push("");
