...
export function generateAgentsMd(employee: EmployeeInput): string {
  ...
  parts.push("");
  parts.push("### Option 1: External App (deploy to Vercel/Netlify/etc, then register the URL)");
  parts.push("```bash");
  ...
  parts.push("    \"hostingMode\": \"external\",");
  parts.push("    \"url\": \"https://my-app.vercel.app\",");
  ...
  parts.push("```");
  parts.push("");
  parts.push("### Option 2: Internal App (deploy HTML directly — we host it for you)");
  parts.push("Build a single-page HTML file with inlined CSS/JS, then deploy it. We serve it at a unique URL automatically.");
  parts.push("");
  parts.push("**Step 1: Register the app**");
  parts.push("```bash");
  parts.push("curl -s -X POST \"$BLITZ_API_URL/employee/apps\" \\");
  parts.push("  -H \"Authorization: Bearer $OPENCLAW_GATEWAY_TOKEN\" \\");
  parts.push("  -H \"Content-Type: application/json\" \\");
  parts.push("  -d '{");
  parts.push("    \"name\": \"Dashboard Analytics\",");
  parts.push("    \"description\": \"Real-time analytics dashboard for team metrics\",");
  parts.push("    \"emoji\": \"📊\",");
  parts.push("    \"type\": \"webapp\",");
  parts.push("    \"hostingMode\": \"internal\",");
  parts.push("    \"shared\": true");
  parts.push("  }'");
  parts.push("```");
  parts.push("This returns the app with an `id`. Use that id to deploy.");
  parts.push("");
  parts.push("**Step 2: Deploy HTML content**");
  parts.push("```bash");
  parts.push("# Build your HTML file first, then deploy it:");
  parts.push("HTML_CONTENT=$(cat my-app.html | jq -Rs .)");
  parts.push("curl -s -X POST \"$BLITZ_API_URL/employee/apps/APP_ID/deploy\" \\");
  parts.push("  -H \"Authorization: Bearer $OPENCLAW_GATEWAY_TOKEN\" \\");
  parts.push("  -H \"Content-Type: application/json\" \\");
  parts.push("  -d \"{\\\"htmlContent\\\": $HTML_CONTENT}\"");
  parts.push("```");
  parts.push("The response includes the live `url` where the app is served. Each deploy increments the version.");
  parts.push("");
  parts.push("**Or register + deploy in one step** (provide `htmlContent` during registration):");
  parts.push("```bash");
  parts.push("HTML_CONTENT=$(cat my-app.html | jq -Rs .)");
  parts.push("curl -s -X POST \"$BLITZ_API_URL/employee/apps\" \\");
  parts.push("  -H \"Authorization: Bearer $OPENCLAW_GATEWAY_TOKEN\" \\");
  parts.push("  -H \"Content-Type: application/json\" \\");
  parts.push("  -d \"{\\\"name\\\": \\\"My App\\\", \\\"type\\\": \\\"webapp\\\", \\\"hostingMode\\\": \\\"internal\\\", \\\"htmlContent\\\": $HTML_CONTENT}\"");
  parts.push("```");
  parts.push("");
  parts.push("**App types:** `script` | `skill` | `webapp` | `api` | `tool`");
  parts.push("**Hosting modes:** `external` (you provide the URL) | `internal` (we host it, max 5MB HTML)");
  parts.push("");
  parts.push("### Option 3: Full-Stack App (HTML frontend + serverless API functions)");
  parts.push("Deploy both a frontend AND backend API functions. Functions run in sandboxed V8 isolates — like Vercel/Cloudflare Workers.");
  parts.push("");
  parts.push("**Deploy with server functions:**");
  parts.push("```bash");
  parts.push("curl -s -X POST \"$BLITZ_API_URL/employee/apps/APP_ID/deploy\" \\");
  parts.push("  -H \"Authorization: Bearer $OPENCLAW_GATEWAY_TOKEN\" \\");
  parts.push("  -H \"Content-Type: application/json\" \\");
  parts.push("  -d '{");
  parts.push("    \"htmlContent\": \"<!DOCTYPE html><html>...</html>\",");
  parts.push("    \"serverFunctions\": {");
  parts.push("      \"GET /api/items\": \"export default async (req) => { return [{ id: 1, name: \\\"Item 1\\\" }] }\",");
  parts.push("      \"POST /api/items\": \"export default async (req) => { return { created: true, data: req.body } }\",");
  parts.push("      \"GET /api/items/:id\": \"export default async (req) => { return { id: req.query.id } }\"");
  parts.push("    },");
  parts.push("    \"envVars\": {");
  parts.push("      \"API_KEY\": \"sk-...\",");
  parts.push("      \"DATABASE_URL\": \"postgres://...\"");
  parts.push("    }");
  parts.push("  }'");
  parts.push("```");
  parts.push("");
  parts.push("**How server functions work:**");
  parts.push("- Each function is a JS module with a default export: `export default async (req) => { ... }`");
  parts.push("- The `req` object has: `{ method, path, query, headers, body }`");
  parts.push("- Path params (`:id`) are available in `req.query` (e.g., `req.query.id`)");
  parts.push("- `env` object is available globally with your env vars");
  parts.push("- Return a value directly (auto-wrapped as JSON) or `{ status, headers, body }` for full control");
  parts.push("- `console.log()` works — logs appear in response header `x-function-logs`");
  parts.push("- Functions run in isolated V8 sandboxes (128MB memory, 5s timeout, no filesystem/network access)");
  parts.push("- Functions are called at: `{app_url}/api/...` (e.g., if your app is at `/app/abc-123`, functions are at `/app/abc-123/api/items`)");
  parts.push("");
  parts.push("**Frontend calling server functions:**");
  parts.push("```html");
  parts.push("<script>");
  parts.push("  // From your deployed HTML, call your own API functions:");
  parts.push("  const res = await fetch('/app/APP_ID/api/items');");
  parts.push("  const items = await res.json();");
  parts.push("</script>");
  parts.push("```");
  parts.push("");
  ...
  parts.push("  -H \"Authorization: Bearer $OPENCLAW_GATEWAY_TOKEN\" | jq '.apps[] | {name, type, url, hostingMode, deployVersion}'");
  parts.push("```");
  parts.push("");
  parts.push("**Update an app:** PATCH `/employee/apps/:appId`");
  parts.push("**Deploy new version:** POST `/employee/apps/:appId/deploy` with `htmlContent` and/or `serverFunctions`");
  ...
  parts.push("");
  parts.push("**5. REGISTER & DEPLOY APPS** — When you build a useful tool, script, or web app, register it via POST `/employee/apps`. For web apps, use `hostingMode: \"internal\"` and deploy HTML via POST `/employee/apps/:id/deploy` — we host it at a unique URL automatically. Or use `hostingMode: \"external\"` and provide a Vercel/Netlify URL. Before building something new, check if a teammate already built an app for it via GET `/employee/apps`.");
  parts.push("");
