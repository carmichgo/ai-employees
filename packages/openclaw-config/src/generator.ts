...
export function generateSoulMd(employee: EmployeeInput): string {
  ...
  parts.push("You can create, generate, and share images across all conversation channels (web chat, Slack, Discord, etc.). The system automatically detects images in your responses and delivers them to the user.");
  parts.push("");
  parts.push("**How image sharing works:**");
  parts.push("- Save any image to your workspace: `/home/node/.openclaw/workspace-main/` or `/home/node/.openclaw/workspace/`");
  parts.push("- **Always include the full file path** in your response text — the system uses this to detect and deliver the image");
  parts.push("- Example: 'Here is the screenshot: /home/node/.openclaw/workspace-main/screenshot.png'");
  parts.push("- The system automatically converts workspace paths to viewable URLs in web chat");
  parts.push("- For Slack and other channels, the system automatically uploads the image file so it appears inline");
  parts.push("- Browser screenshots are saved to `/home/node/.openclaw/media/browser/` and can also be shared by referencing their full path");
  parts.push("");
  ...
  parts.push("- Always save generated images to your workspace before sharing — never reference temporary or in-memory images");
  parts.push("- Use descriptive filenames (e.g., `monthly-report-chart.png`, `logo-draft-v2.png`) so the user knows what the image is");
  parts.push("- When sharing multiple images, mention each file path on its own line for clean rendering");
  parts.push("- Include a brief text description alongside the image so the user has context even before the image loads");
  parts.push("- Supported formats: PNG, JPEG, GIF, WebP, SVG, BMP");
  parts.push("");
