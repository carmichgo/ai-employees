...
export function generateAgentsMd(employee: EmployeeInput): string {
  ...
  if (isExpertTier) {
    ...
    parts.push("## Smart Task Routing (CRITICAL — Cost Control)");
    parts.push("");
    parts.push("You run on Sonnet. You are extremely capable and handle virtually ALL tasks yourself. You have an Expert agent (Opus) available, but it costs 5-10x more per request. Escalating unnecessarily wastes money.");
    parts.push("");
    parts.push("**You (Sonnet)** — handle 98%+ of all tasks. You are the workhorse. Almost nothing requires escalation.");
    parts.push("");
    parts.push("**Expert (Opus)** — `" + expertAgentId + "`. Reserved for rare, exceptional cases only. Think of it as a last resort, not a convenience.");
    parts.push("");
    parts.push("### YOU handle ALL of the following (no escalation):");
    ...
    parts.push("- Code writing, debugging, scripts, automation, code reviews");
    ...
    parts.push("- Strategic analysis, planning, brainstorming");
    parts.push("- Long-form writing including memos, proposals, blog posts");
    parts.push("- Data analysis, comparisons, decision frameworks");
    parts.push("- Multi-step tasks with clear logic");
    parts.push("- ANY task where you can produce a good result — even if imperfect");
    parts.push("");
    parts.push("### Escalate to Expert (Opus) ONLY when ALL of these are true:");
    parts.push("1. You have already attempted the task yourself and your output is clearly inadequate");
    parts.push("2. The task requires reasoning that is fundamentally beyond your capability (not just hard — impossible for you to do well)");
    parts.push("3. The stakes are high enough that the 5-10x cost increase is justified");
    parts.push("");
    parts.push("### Realistic examples of when to escalate (rare):");
    parts.push("- You tried to debug a deeply complex multi-system issue and failed after genuine effort");
    parts.push("- A task requires maintaining intricate logical consistency across many interdependent constraints simultaneously");
    parts.push("- You produced output that is clearly wrong and you cannot figure out why");
    parts.push("");
    parts.push("### DO NOT escalate for:");
    parts.push("- Tasks that seem complex but have clear instructions — just follow the instructions");
    parts.push("- Writing tasks — you write well, even for important documents");
    parts.push("- Strategic analysis — you can reason about tradeoffs effectively");
    parts.push("- Code tasks — you are a strong coder, handle them yourself");
    parts.push("- Tasks you haven't even attempted yet — always try first");
    parts.push("- Tasks where you're unsure if Opus would do better — if you're unsure, it won't");
    parts.push("");
    parts.push("### How to escalate (when genuinely needed):");
    parts.push("Delegate to `" + expertAgentId + "` with clear context about what you tried and why it failed. Relay the result back to the user.");
    parts.push("");
    parts.push("**The golden rule:** ALWAYS attempt the task yourself first. Only escalate after you've tried and demonstrably failed. If your output is decent but not perfect, ship it — don't escalate for marginal improvement. Opus should be used a few times per week at most, not per day.");
    parts.push("");
  }
